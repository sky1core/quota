# quota — 멀티 에이전트·멀티 계정 사용환경 관리

## 개요

quota는 Claude Code와 Codex CLI의 멀티 에이전트·멀티 계정 사용환경을 관리하는 Go 도구다.
핵심 목표 중 하나는 에이전트나 계정을 전환해도 동일한 공통 지침·스킬·정책을 사용하는 환경을 구축하는 것이다.
기본 계정과 등록된 추가 계정 모두 이 목표의 지원 대상이다.

쿼터 조회·표시와 잔여량 기반 계정 선택·실행 위임도 이 목표의 일환이다.
사용 가능한 에이전트·계정의 잔여량을 파악하고 작업을 배정할 때에도 공통 사용환경을 유지하는 것을 목표로 한다.
...
공통 지침·스킬·정책, 쿼터 조회·표시, 계정 선택·실행 위임은 다음 원칙을 따른다.

- **동일성**: 같은 프로젝트의 공통 원본·공통 설정이 같으면, 지원 에이전트나 기본·등록된 추가 계정 사이의 전환으로 공통 지침·스킬·정책의 적용 결과가 달라지지 않아야 한다. 런타임별 연결 방식은 달라도 공통 기능을 조용히 누락해서는 안 된다.
- **비간섭**: 한 계정의 준비·실행이 다른 계정도 사용하는 공유 파일을 자기 계정에만 맞게 바꾸거나 다른 계정의 환경을 훼손해서는 안 된다. 공유 생성물의 선택과 내용은 공통 입력에 따라 결정하며, 마지막으로 실행한 계정이나 준비 순서에 따라 달라져서는 안 된다.
- **계정별 상태 분리**: 인증·로그인 상태·쿼터 값·사용 가능한 모델은 계정별로 구분한다. 쿼터 조회·선택·위임은 동일한 계정 식별을 유지하며, 선택된 계정에도 공통 사용환경을 적용한다. 동일한 환경이 계정별 상태나 제공 기능까지 같은 값으로 만드는 것을 뜻하지는 않는다.
- **지원 범위와 가용성 구분**: 기본 계정과 등록된 추가 계정은 모두 지원 대상이다. 특정 계정의 로그아웃·만료·미사용은 현재 가용성이나 검증 조건이며 지원 제외 사유가 아니다. 런타임 제약·설정 충돌로 공통 환경을 제공할 수 없거나 확인하지 못하면 대상과 이유를 명시하고, 부분 적용·확인 불가를 전체 성공으로 표시하지 않는다.
- **전환 검증**: 기능별로 명시한 전달 범위와 제약 안에서 같은 프로젝트·공통 원본으로 기본 계정 → 추가 계정 → 기본 계정 및 에이전트 전환 후의 적용 결과와 공유 생성물 비간섭을 확인한다. 계정별 설치 성공만으로 동일한 사용환경이 검증됐다고 판단하지 않는다.
- **설정 동일**: quota가 계정마다 설치하는 항목(hook·Codex trust)은 기본 계정과 등록된 추가 계정에 같은 명령·내용으로 설치하며, 계정 경로 인자만 그 계정의 것으로 채운다. quota는 계정 사이에 설정 차이를 만들거나 요구하지 않으며, 사용자 관리 옵션은 바꾸지 않고 각 계정의 유효 상태를 그대로 판정·보고한다.
...
- 외부 Claude/Codex CLI를 실행할 때는 호출자의 계정·인증·provider·모델(별칭 포함)·세션 제어 설정을 상속하지 않는다. 실행 설정은 quota의 명시 입력과 선택된 계정 설정으로 결정한다. 추가 Claude 계정과 Codex 계정은 quota가 확정한 계정 경로를 자식 프로세스 환경에 명시하고, 기본 Claude는 상속된 `CLAUDE_CONFIG_DIR`를 제거한 뒤 Claude의 builtin default 계정을 사용한다. 프로세스 전역 환경을 임시 변경해 대상 선택을 전달하지 않는다.
- 환경 구성은 provider별 공통 진입점에서 처리한다. Claude의 `CLAUDE_*`·`ANTHROPIC_*`, Codex의 `CODEX_*`·`OPENAI_*`처럼 에이전트 전용 네임스페이스는 기본적으로 상속하지 않는다. 접두어가 없는 알려진 CLI 실행 제어 항목도 같은 정책으로 제거한다. `select-agent`의 환경 설정·제거 권고도 이 정책을 따른다.
...
| `quota-cli agent hooks plan [--runtime=all\|claude\|codex] [--binary <path>] [--json]` | 정책 그룹, quota에 등록된 Claude/Codex 계정별 hook 설치 위치와 evaluator 명령 출력, 파일은 수정하지 않음 |
| `quota-cli agent hooks apply [--runtime=all\|claude\|codex] [--binary <path>]` | quota에 등록된 Claude/Codex 계정별 hook 설정 파일을 백업 후 evaluator hook 설치 |
...
- 실행 환경은 위 **호출 환경 독립성** 계약을 따른다. 추가 Claude 계정과 Codex 계정은 선택된 경로를 주입하고, 기본 Claude 계정은 `CLAUDE_CONFIG_DIR` 없이 builtin default를 사용한다.
...
**`agent hooks` Git/GitHub 기본 정책**:
- `remote-code-ref-mutation`의 기본 보호 대상은 서버에 있는 저장소의 코드·ref(브랜치·태그)·저장소 설정이다. 생성·갱신·삭제·덮어쓰기와 이를 예약하는 동작을 차단한다. 로컬 코드·커밋·ref 변경을 원격 변경으로 간주하지 않는다.
- Git/GitHub에서 원격 코드·ref·저장소 설정 변경 이외의 동작을 추가로 차단하려면 대상·조건·합당한 사유·정상 작업에 미치는 영향을 제시하고 사용자의 명시적 승인을 받아야 한다. 추가 규칙은 원격 보호와 구분하여 개별 설정할 수 있어야 하며, 원격 보호를 켰다는 이유로 함께 활성화하지 않는다. 향후 push 가능성이나 막연한 우회 가능성만으로 로컬 작업·조회 전체를 금지하지 않는다.
- 아래 허용은 이 Git/GitHub 기본 정책의 판정이다. 별도로 승인된 데이터 손실·공유 이력·권한·비밀정보 보호 규칙까지 해제한다는 뜻이 아니다. 추가 규칙이 차단할 때는 실제 적용한 규칙과 사유를 반환한다.

| 구분 | 기본 판정과 경계 |
|---|---|
| 원격 코드·ref·저장소 설정 쓰기 | `git push`·`send-pack`·`http-push`, PR merge·자동 merge 설정·update-branch·revert, 원격 브랜치·태그 생성/삭제, 원격 저장소 자체의 생성·fork·동기화·삭제, 저장소 공개 범위·쓰기 권한·보안/자동화 설정 변경 등 실제 원격 쓰기 형태는 차단한다. `git clone`처럼 원격을 읽어 로컬에 복사하는 작업은 이 정책의 원격 쓰기가 아니다. 명령 이름이 같아도 조회·취소·검증 전용 형태까지 일괄 차단하지 않는다. |
| 로컬 Git 작업 | `add`, `commit`·`commit --amend`, `pull`, `merge`, `rebase`, `reset`, `filter-branch`, `update-ref`, `replace`, `reflog expire`, 브랜치 생성·변경·복사·이동·삭제, 로컬 태그 변경·삭제, checkout/switch와 `gh pr checkout`은 이 정책에서 허용한다. 로컬 remote-tracking ref 변경도 서버 ref 변경이 아니다. |
| 조회·복구·설정 | Git 상태·로그·diff·설정 조회, `replace --list`, `merge --abort`·`rebase --abort`, Git/gh alias·include 설정의 조회·편집을 허용한다. 설정으로 나중에 실행할 명령과 설정 편집 자체를 구분한다. |
| GitHub 협업 메타데이터 | push 없는 PR 생성, PR/Issue 조회·본문 수정·코멘트·라벨·리뷰, 브랜치 삭제 없는 PR close, `gh issue develop --list`, `gh pr merge --disable-auto`를 허용한다. PR 생성 과정의 push, issue 연계 브랜치 생성, PR close의 원격 브랜치 삭제는 차단한다. |
| GitHub API | `gh api`를 일괄 금지하지 않는다. endpoint·요청 동작·입력에 따라 읽기와 협업 메타데이터 쓰기는 허용하고 코드·ref 쓰기는 차단한다. HTTP POST라는 이유만으로 금지하지 않으며, GraphQL도 조회와 mutation의 실제 대상을 구분한다. |
...
- 판정 결과는 허용·차단·판정불가다. 활성 정책의 보호 대상 실행인지 결정하는 정보가 부족하면 판정불가로 차단(exit code 2)하고, 해석하지 못한 부분과 이유를 반환한다. 명령 이름이나 실행 내용이 동적인 경우가 해당한다. 반면 로컬 작업·조회로 확정된 명령을 단순히 인자에 변수·glob이 있거나 정책 판단과 무관한 옵션을 모른다는 이유로 판정불가로 만들지 않는다. `git add *.go`, `git commit -m "$message"`, `git log "$BRANCH"`, `gh pr view "$NUM"`는 Git/GitHub 기본 정책에서 허용한다. 반대로 원격 코드·ref 쓰기 계열(예: `git push`)에서 `--dry-run`처럼 실행을 검증 전용으로 낮추는 형태는 관련 인자가 모두 정적일 때만 허용이며, 인자에 동적 값이 하나라도 있으면 판정불가다. 동적 인자가 `--` 뒤 데이터 위치라 실제로는 낮춤을 되돌릴 수 없더라도 옵션 경계 분석으로 무해함을 증명하지 않는다. 이는 과차단이 아니라 계약이며, `git push --dry-run origin main`은 허용, `git push --dry-run $ARGS`와 `git push --dry-run -- origin "$REF"`는 판정불가다.
- 파서는 `mvdan.cc/sh/v3` 하나이며 Bash 문법으로 파싱한다. quota는 셸을 재구현하지 않는다: 함수 정의·변수 값·alias·제어 흐름·실제 실행 여부를 추적하거나 추론하지 않는다. 파싱된 실행 statement는 위치(함수 본문·서브셸·조건·루프 본문·백그라운드)와 관계없이 평가하며, 하나라도 차단 또는 판정불가이면 전체를 차단한다. 런타임 셸의 방언 차이(zsh 전용 의미)는 지원 범위 밖이다.
- 지원 실행 wrapper는 `env`, `sudo`, `command`, `builtin`, `exec`, `nohup`, `nice`, `timeout`, `eval`, `sh -c` 계열 셸 interpreter다. Git/GitHub 명령 계열은 `git`, `gh`다. 각 계열의 옵션 문법(값 소비, 결합 short option, `--` 종료)은 그 계열의 표 한 곳에서 해석한다. 미지원 형태가 보호 대상 실행 여부를 결정하지 못하게 하면 판정불가이며, 파싱 오류·해석 실패를 허용이나 원문 반환으로 바꾸지 않는다. 목록 밖 프로그램의 일반 인자는 실행 명령으로 해석하지 않는다. 다만 `source`/`.`·`xargs` 같은 알려진 간접 실행 진입점의 실행 입력은 일반 데이터 인자로 취급하지 않고 다음 간접 실행 계약을 적용한다.
- 간접 실행에서 정적으로 식별할 수 있는 안쪽 명령은 같은 정책으로 평가한다. 숨은 스크립트·미해석 alias/extension·workflow처럼 실행 내용을 확인할 수 없으면 원격 변경이 확인됐다는 사유 대신 판정불가 사유를 반환한다. 이를 확인하려고 셸 상태 추적이나 자동 외부 조회·실행을 추가하지 않는다. 조회·설정 편집·명령 문자열 출력은 간접 실행과 구분한다.
- 지원 확장은 해당 계열의 표에서 처리한다. 원문 문자열 선차단, 보호 검사별 재해석, 명령마다 덧붙이는 예외로 정책 경계를 대체하지 않는다. 보호 대상 허용뿐 아니라 이 계약에서 허용한 정상 작업의 차단도 결함이다.
- 정책 파일의 내장 positive/negative test와 `agent hooks verify`는 위 경계를 검증한다. 테스트에 과차단 기대값이 있다는 이유로 계약을 바꾸지 않는다. 같은 계열에서 로컬/원격, 조회/쓰기, 일반 데이터 인자/실행 인자를 대조하고, 별도 승인 규칙의 효과는 기본 정책과 구분해 검증한다.
...
- Claude 쪽 hook 위치는 각 계정의 `settings.json` 안 `hooks.PreToolUse`/`matcher=Bash`다. Codex 쪽 hook 위치는 각 계정의 `hooks.json` 안 `hooks.PreToolUse`/`matcher=Bash`다.
...
- Claude 지침 로딩 전제: native 지침 채널이 켜져 있고 `project instruction` 모드가 managed-only가 아니면, Claude는 제외되지 않은 `CLAUDE.md`, `.claude/rules/*.md`와 그 `@import`를 읽는다. 기본 모드에서 프로젝트에 CLAUDE 파일이 없으면 `AGENTS.md`를 대신 읽고, 둘 다 읽는 모드에서는 함께 읽는다. Claude에는 `AGENTS.local.md`를 찾는 native 규칙이 없다. quota는 `project instruction` 옵션을 바꾸지 않으며, `status` 진단은 각 계정의 유효 옵션을 그대로 읽어 판정한다.
- 목적: 공용 `AGENTS.md`는 루트·하위 모두 Claude Code와 Codex CLI가 native로 읽고, 개인 `AGENTS.local.md`는 quota가 세션 시작 hook으로 주입한다. 사용자는 `setup` 한 번으로 선택한 runtime의 quota 기본 계정과 quota 설정에 등록된 추가 계정에 hook을 설치하며, 저장소별 준비 명령과 저장소 생성 파일은 없다. 지침 원본 파일명은 고정이고 별도 스크립트·사용자 작성 hook 없이 동작한다.
- `AGENTS.md` 불간섭: quota는 `AGENTS.md`를 import·복사·병합·주입하지 않으며, CLAUDE 파일을 만들거나 수정하지 않는다.
- 지침 파일 배치 계약: 지침 원본은 공용 `AGENTS.md`와 개인 `AGENTS.local.md`뿐이다. 루트·하위 폴더 모두 `CLAUDE.md`·`.claude/CLAUDE.md`·`CLAUDE.local.md`를 두지 않는 것을 권장한다. Claude Code 기본 설정은 작업 위치부터 상위까지 이 파일이 하나라도 있으면(그 계정의 설정 디렉터리 안의 전역 `CLAUDE.md`는 제외) 하위 폴더 `AGENTS.md`를 읽지 않으며, `@AGENTS.md` 한 줄짜리 루트 `CLAUDE.md`도 같다. 다른 계정 설정 디렉터리의 `.claude/CLAUDE.md`가 상위 폴더에 있으면 Claude는 그것을 프로젝트 CLAUDE 파일로 분류한다. quota는 이를 `status` 진단에 반영하며, 그로 인한 `AGENTS.md` 로딩 차이는 quota 범위 밖이다.
- 주입: 각 계정의 세션 시작 hook(`_prepare`)은 hook 입력 `cwd`가 속한 Git 저장소의 primary 루트(bare 저장소는 bare 루트)에 있는 `AGENTS.local.md` 전문을 additionalContext로 넣는다. linked worktree도 primary의 파일을 읽는다. 파일이 없거나 Git 저장소 밖이면 아무것도 넣지 않는다. Claude와 Codex 모두 source가 startup·clear·compact일 때 넣고 resume에는 넣지 않는다(이전 전달이 transcript에 남아 있다). source가 없거나 알 수 없는 값이면 넣지 않는다. SubagentStart와 매 턴 입력에는 붙이지 않는다.
- 원본 검사: `AGENTS.local.md`는 symlink가 아닌 정규 파일이어야 하고, 유효한 UTF-8이며 NUL이 없고 8MiB 이하여야 한다. 위반이면 본문 대신 notice를 넣는다. 원본 오류를 부재로 취급하지 않는다.
- 단일 출력: 각 런타임에는 지침을 주입하는 SessionStart hook 하나만 설치한다. quota는 `AGENTS.local.md` 본문을 분할하거나 잘라서 출력하지 않으며, 원본 검사와 전달 한도에 걸리지 않으면 하나의 additionalContext에 전문을 넣는다. Codex hook은 `additionalContextLimit = 0`으로 설정한다.
- Claude 전달 한도: 머리줄·notice를 포함한 최종 additionalContext가 UTF-16 단위 10,000자를 넘으면, 전문 대신 짧은 오류를 맨 앞에 전달하고 기존 notice는 뒤에 보존한다. 오류는 지침 hook 출력의 한도 초과로 전체 context를 전달하지 못함을 사용자에게 알리고 정상 로드로 보고하지 말라고 에이전트에게 지시하며, 원본 파일이 없는 경우에도 맞는 표현을 사용한다. 이는 보고 지시이며 런타임 강제 중단은 아니다. `status`도 본문과 머리줄이 이 한도를 넘으면 문제로 보고한다. 본문을 줄이거나 나눠서 이 제약을 우회하지 않는다.
- Claude 입력 검사: UserPromptSubmit hook 하나가 현재 primary `AGENTS.local.md`의 원본과 본문·머리줄의 10,000 UTF-16 단위 한도를 검사한다. 초과·원본 오류·검사 실패이면 실패 사실과 원인을 모델의 입력 context로 전달하고, 사용자에게 직접 설명하고 도구 호출과 요청 작업을 보류하며 문제 해결 뒤 새 세션이 필요함을 안내하도록 지시한다. 분할·잘라내기·한도 우회를 제안하거나 지침 파일을 임의 수정하지 않도록 지시한다. 제품의 보장 범위와 성공 기준은 오류·원인·설명 및 작업 보류 지시를 모델에 전달하는 것까지다. 모델의 지시 준수, 런타임 강제 차단, `-p`의 오류 종료는 보장하지 않는다. 정상 범위·빈 파일·파일 부재·Git 저장소 밖이면 통과한다. 본문 재주입·생성물 정리·상태 파일 저장은 하지 않는다. 대화형과 `claude -p` 모두 모델 context에 오류와 지시가 전달되는지를 검증하며, 터미널 경고나 종료 코드만으로 전달 성공을 판정하지 않는다. 이 검사는 과거 SessionStart 전달 성공이나 일회성 정리 notice까지 보증하지 않으며, 파일을 고친 뒤 지침을 다시 로드하려면 새 세션을 시작해야 한다.
- `setup [--agent=all|claude|codex] [--dry-run]`는 선택한 runtime의 기본 계정과 quota 설정에 등록된 추가 계정 모두에 hook을 설치한다. 대상 산출에는 호출 환경의 `CLAUDE_CONFIG_DIR`/`CODEX_HOME`을 사용하지 않는다. hook 명령은 모든 계정에 동일하며 계정 경로 인자를 갖지 않는다. 이전 버전의 quota 지침 hook(분할 hook, WorktreeCreate·WorktreeRemove 항목, `_hook`, 계정 경로 인자가 있는 `_prepare`, 셸 스크립트 hook)은 교체·제거한다. Codex 대상이 있으면 설치한 quota hook의 현재 native hook hash만 각 계정 `config.toml`의 trust state에 동기화하고, 무관 hook·설정·주석·native 신뢰 상태는 보존한다. 전역 git ignore 파일은 더 이상 관리하지 않는다. dry-run은 같은 사전 검사와 변경 계획만 출력한다.
- `uninstall [--agent=all|claude|codex] [--dry-run]`은 계정 hook을 제거한다. 저장소의 파일은 건드리지 않는다.
- Claude 초기화: `setup`은 hook 설치 후 대상 Claude 계정마다 모델 대화 없는 초기화 명령을 실행하고 결과를 안내한다. 준비 범위는 저장소별이 아니라 계정 설정 환경별이며, 별도 준비 상태를 저장하거나 native 로딩을 성공 조건으로 검사하지 않는다. 초기화 실패는 경고만 하고 hook 설치 성공을 오류 종료로 바꾸지 않는다. `dry-run`은 실행 계획만 안내하며 `status`·`uninstall`은 초기화하지 않는다.
- 이전 생성물 정리: 이전 버전이 저장소에 만든 `.claude/rules/quota-instructions.md`, `AGENTS.override.md`, `.claude/AGENTS.md`, `.claude/CLAUDE.md`, linked worktree 복사본은 `~/.config/quota/instructions/<common-dir 경로 해시>.json`에 기록된 해시와 현재 내용이 일치하는 것만 hook 실행 시 제거하고, 기록 파일을 삭제한다. 일치하지 않는 파일은 남기고 notice로 알린다. `status`는 남은 기록과 파일을 보고만 한다. quota는 `.git/` 아래에 쓰지 않는다.
- `status [dir] [--agent=all|claude|codex]`는 모델을 실행하지 않고 다음을 보고한다. 계정별 hook 설치 상태(각 런타임 SessionStart 1개와 Claude UserPromptSubmit 1개의 고정 명령·실행 설정, `disableAllHooks`, Codex trust state), 저장소 primary의 `AGENTS.local.md` 존재·원본 검사, native `AGENTS.md` 읽기를 막는 조건(작업 위치부터 상위의 `CLAUDE.md`·`.claude/CLAUDE.md`·`CLAUDE.local.md`, `AGENTS.md`를 읽지 않는 유효 project instruction 모드)은 경고. 설정됨·차단됨·확인 불가를 구분하며 일부만 준비됐거나 확인 불가이면 전체 성공으로 응답하지 않는다.
- 사용자-facing `agent instructions` 검증은 `status`까지다. 모델 호출로 실제 지침 전달을 확인하는 live canary는 일반 사용자 명령으로 제공하지 않는다. 기본 기능 검증은 프로덕션 코드의 파일·설정 계약을 검사하는 로컬 테스트로 수행한다. 사용자 계정의 Claude/Codex quota를 소비하는 검증을 정상 사용 흐름에 넣지 않는다.
...
- 내부 진입점 `_prepare`는 고정된 공급자/이벤트 조합(`claude`·`codex`: SessionStart, `claude`: UserPromptSubmit)만 받으며 임의 파일·명령 실행 입력은 받지 않는다.

**`agent skills` — 스킬 일괄 관리**:

- `quota-cli agent skills install <source> [--skill <name>]`, `link [<name>]`, `list`, `remove <name>`은 Claude Code 다중 계정과 Codex CLI의 스킬을 일괄 관리한다. `--scope=global|repo`의 기본값은 `global`이며, `repo`는 현재 Git 저장소의 루트를 대상으로 한다. `--skill`을 생략하면 소스에 포함된 스킬 전체를 설치한다. 각 명령은 `--json` 출력을 지원한다.
- 스킬 기준 디렉터리로 범위별 `.agents/skills` 하나를 사용하며, 바로 아래 스킬 이름별로 디렉터리를 하나씩 둔다. 사용자 범위는 홈 디렉터리, 저장소 범위는 저장소 루트를 기준으로 하며 Codex는 이 위치를 직접 읽는다. Claude는 사용자 범위에서 기본 계정과 quota에 등록된 추가 계정 각각의 `skills/<name>`, 저장소 범위에서 `<repo>/.claude/skills/<name>`이 해당 스킬 디렉터리를 가리키는 심링크다. 실제 경로가 같은 계정 스킬 디렉터리는 하나의 대상으로 다루며, 기준 위치와 같으면 링크를 만들지 않는다. 호출 환경의 `CLAUDE_CONFIG_DIR`·`CODEX_HOME`으로 대상을 바꾸지 않는다.
- Codex는 사용자 범위에서 계정별 `<CODEX_HOME>/skills`, 저장소 범위에서 `<repo>/.codex/skills`도 읽으므로, 그 위치에 같은 이름이 있으면 중복으로 보고한다. 이 위치에는 설치하거나 링크를 만들지 않는다.
- 소스 취득에는 버전을 고정한 Vercel `skills` CLI를 사용한다. 설치에는 해당 버전이 요구하는 Node.js와 npm이 필요하며, 의존성 부재·소스 취득 실패를 다른 설치 수단으로 우회하지 않는다. `link`·`list`·`remove`에는 소스 취득이 필요하지 않다.
- `install`은 소스를 기준 경로에 한 번만 복사하고 Claude 경로에 심링크를 만든다. 기준 경로가 이미 있거나 Claude 경로·Codex 중복 위치가 다른 항목으로 차 있으면 내용이 같아도 인수·교체하지 않고, 충돌을 보고한 뒤 아무것도 바꾸지 않는다.
- `link`는 기준 경로에 이미 있는 스킬(이름 생략 시 전체)에 대해 빠진 Claude 심링크를 만든다. 사용자가 기준 경로에 직접 만든 스킬과 새로 등록한 계정이 이 명령으로 연결된다. 다른 항목이 차 있는 경로는 바꾸지 않고 충돌로 보고한다.
- 스킬 수정은 기준 경로의 파일을 직접 편집한다. 수정 여부는 어떤 명령의 조건도 아니다.
- `remove`는 지정 범위에서 그 이름의 기준 경로·Claude 경로·Codex 중복 위치를 모두 백업 위치로 옮긴다. 심링크는 링크 자체만 옮기고 목적지를 따라가지 않는다. 백업할 수 없는 항목(다른 파일시스템의 디렉터리 등)이 하나라도 있으면 아무것도 옮기지 않는다.
- `list`는 별도 기록 없이 실제 파일시스템을 읽어 스킬명별 기준 경로와 실제 경로, Claude 경로와 연결 상태, 부재·충돌·깨진 링크·Codex 중복을 보고한다. 설치·연결·삭제가 일부만 적용되거나 충돌로 적용되지 않았거나, 상태를 확인할 수 없으면 대상별 결과와 사유를 출력하고 non-zero로 종료한다. `list`는 충돌을 보고만 하며 조회 실패일 때만 non-zero다.
- 자동 동기화·업데이트, 서로 다른 기존 사본의 병합·이전, 저장소 범위 파일의 Git 추적·공개 여부, 실제 세션에서의 스킬 로딩 검증은 다루지 않는다.
...
- 알려진 제약: 번들 심링크는 마지막으로 시작한 바이너리를 가리키고, launchd plist는 등록 시점의 경로를 그대로 가리킨다(번들 도입 후 등록하면 번들 경로, 그 전에 등록했으면 당시의 바이너리 경로이며 기존 plist를 자동 이관하지 않는다). 그래서 go-install 경로가 아닌 바이너리(예: dev 체크아웃 빌드)를 가리키는 plist는 다음 launchd 재시작 때 그 바이너리를 다시 띄우고 심링크도 그쪽으로 돌아간다. 업데이트 핸드오버는 심링크를 go-install 경로로 되돌리지만 plist를 고치지는 않는다 — Start at Login을 다시 켜서 등록하면 번들 경로로 고쳐진다.

**시스템 식별용 앱 번들**: 단일 실행파일은 LaunchServices에 번들 ID 없이 등록되어 macOS 26의 메뉴 막대 설정이 아이템을 식별·복원하지 못한다. quota-bar는 시작 시 `~/.config/quota/quota-bar.app` 껍데기(`Info.plist` + 실행 바이너리로의 심링크 `Contents/MacOS/quota-bar`)를 만들거나 갱신하고 항상 그 경로로 실행된다. 일반 실행은 번들 경로로 데몬 자식을 띄우고, 맨 바이너리로 데몬 진입한 경우(옛 launchd plist, 업데이트 핸드오버)는 같은 pid로 번들 경로에 `exec`한다. 로그인 시 실행 plist도 번들 경로를 가리킨다. 껍데기를 만들지 못하면 시작하지 않는다. 메뉴바 아이템의 표시/숨김은 macOS 설정에 맡기며 앱이 되돌리지 않는다.
...
- 예정 시각에 PC 입력이 지정 시간 이상 없고, 최근 지정 시간 안에 실제 작업 활동이 확인된 세션만 후보로 삼는다. CLI가 해당 세션을 열고 살아 있으며 평상시 입력 대기 중임을 확인해야 한다. 작업 중, 승인·질문 등 사용자 응답을 기다리는 작업, 제출되어 대기 중인 입력이나 백그라운드 작업이 있는 세션, 상태가 불명확한 세션은 제외한다. 전송 전 편집기 초안은 그대로 보존한다. 로그 수정 시각만으로 활동이나 실행 상태를 추론하지 않는다.
...
- 예약일에는 예정 시각 전 `activityMinutes` 구간부터 해당 일정의 처리가 끝날 때까지, 외부 전원에서만 시스템 자동 잠자기를 억제한다. 화면 꺼짐은 허용한다. 기능을 끄거나 앱이 종료되면 억제를 해제한다. 배터리 전환 시 억제는 효력을 잃으며, 전원·운영체제 상태 때문에 잠자기 억제를 확인할 수 없으면 이를 사용자에게 표시한다. 사용자가 직접 잠자기를 선택하거나 덮개를 닫는 동작까지 막는다고 보장하지 않는다.
...
2. 프로세스 환경은 `EnvForConfigDir`에서 **호출 환경 독립성** 계약에 따라 구성한다. `CLAUDECODE`를 비롯한 호출자 실행 설정을 제거하고, 추가 계정에만 확정된 `CLAUDE_CONFIG_DIR`를 주입한다. 기본 계정에는 새 값을 넣지 않는다.
