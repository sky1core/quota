...
```json
{ "selected": ["codex_5h", "claude_session"], "showResetTime": false }
```

- `selected` (string 배열): 상단 바에 표시할 체크된 항목 키.
- `showResetTime` (bool, optional, 기본 false): true면 각 항목의 리셋을 남은시간 대신 절대 시각(`FormatResetAt`, 예: `Jul 6 15:04`)으로 표시. 메뉴의 `Reset as clock time` 체크로 토글하며, 즉시 저장 후 재조회 없이 메뉴만 다시 그린다. 상단 바(퍼센트 전용)에는 영향 없다.
