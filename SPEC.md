...
- `keepalive` 설정의 기본값은 아래와 같다. 토글은 즉시 저장·반영하며, 다른 값은 설정 파일에서 변경하고 앱을 재시작한다. 요일은 `Sun`부터 `Sat`, 시각은 현지 시각 `HH:MM`, 두 시간 범위는 1–1440분, 메시지는 공백을 제외한 내용이 있는 8192바이트 이하 문자열이다.
