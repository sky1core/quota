...
- `local-system-secret-safety` 차단 그룹에는 `rm`/`rmdir`/`unlink`, `mkfs*`/`newfs*`/`fdisk`, `dd if=*`/`dd of=*`, `sudo`/`doas`/`su`, `shutdown`/`reboot`/`poweroff`/`halt`/`init 0`/`init 6`, `killall`/`pkill`, 여러 PID 또는 음수 PID를 대상으로 하는 `kill`, `chmod`/`chown`/`chgrp`, `gh auth token`, `gh auth status --show-token`/`-t`가 포함된다. 단일 양수 PID를 대상으로 하는 `kill <pid>`·`kill -TERM <pid>`·`kill -s TERM <pid>`와 `trash ...`는 이 그룹의 허용 대조군이다.
