...
- 판정 결과는 허용·차단·판정불가다. 활성 정책의 보호 대상 실행인지 결정하는 정보가 부족하면 판정불가로 차단(exit code 2)하고, 해석하지 못한 부분과 이유를 반환한다. 명령 이름이나 실행 내용이 동적인 경우가 해당한다. 반면 로컬 작업·조회로 확정된 명령을 단순히 인자에 변수·glob이 있거나 정책 판단과 무관한 옵션을 모른다는 이유로 판정불가로 만들지 않는다. `git add *.go`, `git commit -m "$message"`, `git log "$BRANCH"`, `gh pr view "$NUM"`는 Git/GitHub 기본 정책에서 허용한다.
