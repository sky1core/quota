...
- 시작 시 status item을 `autosaveName=quota-bar`로 고정하고 표시를 강제한다. macOS 메뉴 막대 설정이 앱 defaults에 저장한 숨김(`NSStatusItem VisibleCC …`)은 재시작을 넘어 유지되지 않는다. 실행 중에 시스템 설정에서 끄면 그 실행 동안은 숨겨지며 다음 시작에 다시 보인다. 아이템을 없애려면 앱을 종료한다.
