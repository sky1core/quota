...
{
  "windows": [
    { "windowMins": 300,   "label": "5h", "left": 85, "resetsIn": "2h 30m", "resetsAt": "2026-07-06T15:04:00+09:00" },
    { "windowMins": 10080, "label": "7d", "left": 70, "resetsIn": "6d 19h", "resetsAt": "2026-07-18T09:00:00+09:00" }
  ],
...
**윈도우 (`windows`, 자기서술 목록)**: Codex 응답의 rate-limit 윈도우는 `primary`/`secondary` 위치로
오지만, Codex는 **어떤 윈도우를 어느 위치에 실을지, 어떤 윈도우가 존재하는지 시점에 따라 바꾼다**(예:
모델 출시 전후로 5h 윈도우가 사라지고 주간 윈도우가 primary로 이동). 그래서 계약은 위치도, 범위 버킷도
아니라 **각 항목이 자기 실제 기간(`windowMins`)과 그 기간에서 곧이곧대로 만든 표시명(`label`)을 들고
있는 목록**이다. 소비자는 목록을 순회해 `label`로 표시만 하면 되고, Codex가 윈도우 구성을 바꿔도
코드가 깨지지 않는다.

각 윈도우 항목:
- `windowMins` (int): 응답의 `windowDurationMins` 원본(분). 이 윈도우가 어떤 창인지의 **사실(fact)**이다.
- `label` (string): 표시명을 **`windowMins`에서 진실하게** 만든 것(`windowLabel`) — 300→`5h`, 600→`10h`, 10080→`7d`, 1440→`1d`, 43200→`30d`, 90→`1h 30m`. **범위를 뭉개 고정 라벨을 붙이지 않는다**: 5시간이 아닌 창을 `5h`라 부르지 않는다. `windowMins` 없이 라벨을 지어내지 않는다.
...
규칙:
- 목록 순서는 **실제 기간 오름차순**(짧은 창 먼저)으로 고정한다(응답 위치와 무관, 표시 순서 안정).
- **응답에 존재하는 윈도우만** 목록에 들어간다. 5h가 없으면 그 항목 자체가 없다. 표시 가능한 윈도우가 하나도 없으면 `windows` 키를 생략한다.
- `windowDurationMins`가 없는 윈도우는 진실한 라벨을 만들 수 없어 **생략**한다(위치/버킷 기반 라벨링이 이 재설계로 제거하려는 버그다).
- 같은 `windowMins`가 두 번 오면(동일 윈도우 중복) 먼저 온 것만 남긴다. 서로 다른 기간은 서로 다른 창이므로 둘 다 남긴다(버킷으로 합치지 않는다).
- 출력에 `windowMins`+`label`이 내장되므로 외부 소비자는 별도 표 없이 목록 순회만 하면 된다. quota-bar는 systray가 런타임에 행을 못 만들어 슬롯을 미리 만들어야 하므로 **내부적으로만** 기간→슬롯 버킷을 쓰지만, 화면 라벨은 항상 이 `label`이다(버킷명이 라벨로 새지 않는다).
...
- 각 그룹은 자기 **윈도우 체크박스**(아래 "Codex 동적 창 슬롯" 참조) + 자기 **Reset credits(초기화권)** 부모 행·서브메뉴(아래 참조)를 가진다. 초기화권은 계정별로 독립이며 서로 섞이지 않는다.
- 같은 과금 계정을 다른 `CODEX_HOME`에 로그인한 경우, quota 숫자·초기화권은 서버측 계정 단위라 두 그룹이 동일하게 나온다(로컬 격리만 다름).

**메뉴 항목 키 스킴** (설정 파일 하위호환):
- 키 형식은 `<provider>_<suffix>`이며 `provider`는 계정 key(`claude`, `claude-2`, …, `codex`, `codex-2`, …)다. 계정 key는 `^claude-\d+$`/`^codex-\d+$`(또는 기본 `claude`/`codex`)라 `_`를 포함하지 않으므로, 첫 `_`가 항상 provider와 suffix를 가른다.
- Claude 기본 계정 키는 **기존 그대로 유지**한다: `claude_session`, `claude_weekly_all`, `claude_extra_1`~`claude_extra_3`.
- Codex 윈도우 **슬롯 키**는 내부 duration 버킷이다: `codex_5h`, `codex_daily`, `codex_weekly`, `codex_monthly`. 이는 **선택/슬롯 식별자일 뿐 화면 라벨이 아니다**(화면 라벨은 데이터의 진실한 `label`). 구 `codex_day`(주간 윈도우를 담던 슬롯)는 로드 시 `codex_weekly`로 마이그레이션되어 기존 선택이 유효하게 유지된다.
- 추가 계정은 동일 스킴을 계정 key로 확장한다: Claude는 `claude-2_session`/`claude-2_weekly_all`/`claude-2_extra_1`~, Codex는 `codex-2_5h`/`codex-2_weekly`~.
- 선택 상태(체크)는 슬롯/버킷 기준 키로 저장되므로 모델명·창 재배치가 있어도 유지된다.

**Codex 동적 창 슬롯**:
- 각 Codex 계정 그룹은 내부 슬롯 버킷(`codexWindowSlotKeys`: 5h/daily/weekly/monthly)마다 체크박스 슬롯을 **미리 숨김 상태로** 만든다(systray 런타임 항목 추가 불가 — quota-bar가 버킷을 쓰는 유일한 이유). 이 버킷은 슬롯 주소일 뿐 화면에 뜨지 않는다.
- refresh 시 `windows` 목록의 각 항목을 `codexWindowBucket(windowMins)` 슬롯에 채우고, **행 라벨은 그 항목의 진실한 `label`**(예 `5h`, `7d`)로 그린다 — 버킷명이 아니다. 데이터가 있는 슬롯만 show, 없으면 hide. Codex가 창 구성을 바꿔(5h 소멸 등) 어떤 창이 오든 라벨과 데이터가 어긋나지 않는다.
- 4개 버킷 범위가 duration 전 구간을 덮으므로 `windowMins`가 있는 창은 자기 슬롯에 표시된다(예외: 두 창이 같은 슬롯이면 첫 창만 — 서로 다른 tier라 이론상). `windowMins`가 없어 표시 불가한 창만 생략된다.
...
- Codex 각 그룹은 `windows` 목록을 순회해 각 항목을 **자기 `label`로** 렌더한다(목록이 이미 정식 순서). 키 어휘를 하드코딩하지 않으므로 새로운 윈도우 종류도 그대로 표시된다.
