...
**동작**:
1. `codex app-server` 프로세스를 시작 (stdin/stdout pipe). 상속된 `CODEX_HOME`을 제거하고 quota가 확정한 `CODEX_HOME=<codexHome>`을 하나만 주입한다. 로그인한 home 계정 대신 환경 override가 사용되지 않도록 `CODEX_ACCESS_TOKEN`/`CODEX_API_KEY`/`CODEX_AUTH`/`CODEX_AUTHAPI_BASE_URL`/`CODEX_URL`과 `OPENAI_API_KEY`/`OPENAI_BASE_URL`/`OPENAI_ORGANIZATION`/`OPENAI_PROJECT`를 제거한다. `CODEX_SQLITE_HOME`도 제거해 호출자 세션의 상태 DB 위치가 선택된 Codex home을 덮어쓰지 않게 한다.
2. JSON-RPC 2.0 프로토콜:
