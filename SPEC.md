...
- 공용 `AGENTS.md`와 개인 `AGENTS.local.md`의 탐색·전달·worktree 동기화를 quota가 직접 수행한다. 별도 스크립트·Python·사용자 작성 hook spec 없이 동작하며, 지침 원본 파일명은 고정이다.
- `setup [dir] --agent=all|claude|codex [--local-file=PATH ...] [--dry-run]`은 지정 저장소를 준비하고 선택한 현재 CLI 계정의 연결을 설치한다. 기본 디렉터리는 현재 디렉터리, 기본 agent는 `all`이다. `all`은 상속된 환경으로 선택되는 Claude 계정 하나와 Codex 계정 하나이며, quota의 등록 계정 전체를 수정하지 않는다. dry-run은 같은 사전 검사와 변경 계획만 수행한다.
- `setup --local-file=PATH`는 primary 기준의 상대 파일 경로를 저장소 복사 목록에 등록하며 반복 지정할 수 있다. 기존 등록은 유지되고 같은 경로의 재등록은 중복되지 않는다. 명시한 8MiB 이하의 ignored·untracked 정규 파일만 linked worktree에 복사하며 소유자 실행 권한을 보존한다. 경로 이탈·symlink·예약된 지침 생성물 경로·quota가 자체 갱신하는 메타데이터 경로·`.gitignore` 파일은 거부한다. 대소문자와 Unicode 정규화 기준으로 같은 경로나 겹치는 경로도 거부하며, 원본 파일명은 변경하지 않는다. 추가 파일의 ignore 기록은 Git common-dir에만 저장한다. repository uninstall로 두 공급자를 모두 비활성화하고 관리 복사본을 안전하게 제거하면 등록 목록도 비운다. 이 정리는 원본 파일의 존재를 요구하지 않는다.
- `status [dir] --agent=all|claude|codex`는 원본·관리 생성물의 소유권·최신성·연결·유효 native 설정를 검사한다. 모델을 실행하지 않으며, 설정됨·차단됨·신뢰 대기·확인 불가를 구분한다. 일부만 준비됐거나 확인 불가이면 전체 성공으로 응답하지 않는다.
- `verify [dir] --agent=all|claude|codex`는 status와 같은 전제 검사 후 임시 저장소에서 실제 CLI의 공용/개인 지침 전달과 비전달 대조군을 확인한다. 모델 호출 비용이 발생하며, 검증 성공은 실행한 공급자의 새 메인 세션에 한정한다. 그 성공을 현재 설정 상태에 영속 저장하거나 모델의 지침 준수 강제로 표현하지 않는다.
- Claude는 `CLAUDE.md`의 `@AGENTS.md`와 `CLAUDE.local.md`의 native 로딩을 사용한다. primary의 개인 bridge는 `@AGENTS.local.md`, linked worktree의 개인 bridge는 primary 원본의 관리 복사본이다. SessionStart는 복사본 갱신과 실패 알림만 수행하고 본문을 다시 주입하지 않는다. SubagentStart로 native 전달을 중복하지 않는다. native memory를 생략하는 내장 subagent와 호출별 로딩 override는 전달 검증 범위 밖이다.
- Codex는 개인 원본이 있으면 대상 checkout의 공용 본문과 primary 개인 본문을 결정적으로 병합한 `AGENTS.override.md`를 native로 읽는다. 개인 원본이 없으면 공용 `AGENTS.md`를 native로 읽는다. 같은 본문을 hook이나 매 턴 입력에 다시 붙이지 않는다. 원본과 경로가 같으면 resume 때 추가 전달을 만들지 않는다. 원본 변경에 따른 이전 본문의 잔존은 허용하며, 이전 지침만 선택 삭제하거나 자동으로 압축하지 않는다. native 문서 탐색 변경·명시적 비신뢰·병합본의 유효 문서 예산 초과는 정상 전달로 인정하지 않는다. 하위 디렉터리의 별도 지침은 CLI native 기능이며 이 루트 지침 규약의 검사 범위 밖이다.
- 원본 두 파일은 사용자 소유다. 개인 원본은 Git common-dir에 대응하는 primary에 하나만 두며, bare 저장소에서는 bare 루트에 둔다. linked worktree의 `AGENTS.local.md`는 관리 복사본이다. 생성물은 기록된 소유권과 마지막 생성 내용이 모두 일치할 때만 갱신·제거한다. 생성 시 실제 파일 권한도 기록하고, 기록된 권한이 바뀐 생성물은 보존한다. 권한 기록이 없는 기존 소유권은 본문 검증을 유지하며 다음 setup에서 현재 권한을 기록한다. 사용자 override·출처 불명 파일·사용자가 수정한 생성물은 내용이 같아 보여도 임의로 채택하거나 덮어쓰지 않는다. private 파일의 Git 추적, 지침 원본의 symlink·비정규 파일·잘못된 UTF-8·NUL은 오류다. 개인 생성물은 git-ignored와 소유자 권한을 유지한다.
- 공용 원본 정책 기본값은 `checkout`이며 각 checkout의 공용 파일을 그대로 사용한다. `setup --shared-source=primary`를 명시하면 primary 공용 원본을 관리 복사하며 정책은 같은 Git common-dir에 저장한다. source가 없다고 다른 checkout의 파일을 암시적으로 대신 읽지 않는다. 외부에서 만든 새 worktree의 미추적 공용 파일은 해당 worktree에서 setup해야 하며, native 로딩 이후의 hook 복사를 사전 로딩 성공으로 취급하지 않는다. Codex의 병합본과 로컬 파일은 세션 생성 전에 setup 또는 worktree 준비 단계에서 갱신한다. 원본 변경·외부 worktree 생성 후에는 다음 실행 전에 setup을 다시 수행하며, SessionStart 이후 파일 갱신을 같은 세션의 native 로딩 성공으로 취급하지 않는다.
- Claude WorktreeCreate는 primary 밖에 worktree를 만들고 지침 생성물을 준비한 뒤 경로를 반환한다. WorktreeRemove는 관리한 worktree만 정리하며 사용자 변경·알 수 없는 ignored 파일·변경 검사를 생략하는 index 플래그가 있으면 보존하고 실패한다. 원본과 소유권이 불명확하거나 사용자가 수정한 생성물을 덮어쓰지 않는다.
- Claude 연결은 현재 계정의 `settings.json`에 고정된 quota 명령으로 설치한다. Codex setup은 선택 계정의 정확히 식별된 기존 지침 주입 hook을 제거하며 새 주입 hook을 설치하지 않는다. 다른 저장소의 개인 지침도 native 준비를 마쳐야 하므로 계정 전체에 적용되는 제거 경로를 계획에 표시한다. 무관 설정·hook·주석과 native 신뢰 상태는 보존한다. 명령어의 첫 인자나 부분 문자열로 소유권을 추정하지 않는다.
- 사전 검사는 모든 선택 대상에 대해 쓰기 전에 수행한다. 설정의 최신 읽기·수정·백업·원자적 저장은 대상 잠금 안에서 처리한다. 여러 파일 중 일부만 적용되면 적용 경로와 실패 경로를 보고하고 non-zero로 끝낸다. 재적용은 중복 hook·불필요한 백업·동일 파일 재쓰기를 만들지 않는다. 파일 충돌을 발견하면 어떤 로컬 복사본이나 병합본도 변경하지 않는다. 설정 파일의 별칭을 적용 예정 대상과 대응시킬 수 없으면 사전 검사에서 실패하며, 기존 설정 본문으로 대신 승인하지 않는다. 원본이 같으면 생성 결과와 파일을 유지하고, 원본이 바뀌면 선택한 공급자의 소유권이 확인된 복사본과 병합본을 함께 갱신한다.
- `uninstall --scope=account|repository --agent=all|claude|codex [dir]`은 scope를 반드시 지정한다. account는 해당 CLI 설정 홈의 관리 연결만 제거하며 그 계정이 사용하는 다른 저장소에도 영향을 준다. repository는 같은 Git common-dir의 선택 공급자 지침을 비활성화한 뒤 안전하게 제거 가능한 관리 생성물을 정리한다. 원본과 다른 사용자 hook은 보존한다. 비활성 상태에서도 일반 Claude worktree 생성·경로 반환·안전한 정리는 유지한다. setup은 선택한 공급자를 다시 활성화한다. Codex native 공용 지침 읽기를 중단시키는 명령으로 표현하지 않는다.
- 저장소 상태는 명시적인 enabled 기본값·잠금·원자적 저장·소유자 전용 권한을 사용한다. 손상된 상태를 기본값으로 대신하지 않는다. 내부 hook 진입점은 고정된 공급자/이벤트 조합만 받으며 임의 파일·명령 실행 입력은 받지 않는다.

**`agent overlay` 동작 (기존 spec 호환 명령)**:
...
- 관리 hook은 기대 그룹 조건과 실행 객체를 함께 비교한다. 빈 `matcher`와 생략은 동등하며 문자열 `statusMessage`는 허용한다. 그 밖의 추가 필드(`if`, `args`, `async` 등)는 지원하지 않는 변형으로 보고한다. 이는 런타임에서 실행 불가능하다는 판정은 아니다. 중복 관리 명령도 정상으로 인정하지 않는다. 요구 hook이 있을 때 Claude 사용자 설정의 `disableAllHooks: true`, Codex 사용자 설정의 `features.hooks = false`는 방해 조건이다. Codex는 정식 키가 없을 때만 이전 별칭 `features.codex_hooks`의 활성화 값을 사용하며, 지정된 두 키의 타입은 모두 boolean이어야 한다. 관리자 전용 키인 Claude `allowManagedHooksOnly`와 Codex `allow_managed_hooks_only`를 사용자 설정에 둔 것은 방해 조건으로 취급하지 않는다. 관리자 계층의 실제 정책 적용 여부는 이 검사 범위에 포함하지 않는다. apply는 이 설정을 임의로 켜거나 해제하지 않고 저장 후 재검사에서 방해 조건을 보고한다.
