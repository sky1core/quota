...
- 각 그룹은 자기 `5h`/`Day` 체크박스 + 자기 **Reset credits(초기화권)** 부모 행·서브메뉴(아래 참조)를 가진다. 초기화권은 계정별로 독립이며 서로 섞이지 않는다.
...
- 기본 계정 키는 **기존 그대로 유지**한다: `claude_session`, `claude_weekly_all`, `claude_extra_1`~`claude_extra_3`, `codex_5h`, `codex_day`. 따라서 기존 `quota-bar.json`의 `selected` 값이 그대로 유효하다.
- 추가 계정은 동일 스킴을 계정 key로 확장한다: Claude는 `claude-2_session`/`claude-2_weekly_all`/`claude-2_extra_1`~, Codex는 `codex-2_5h`/`codex-2_day`.
