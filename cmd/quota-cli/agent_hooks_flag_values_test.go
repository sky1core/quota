...
func TestAgentHooksEvalOptionValues(t *testing.T) {
	...
	cases := []struct {
		...
		{`git branch --delete --end-of-options feature`, true},
		{`git tag -m --end-of-options --delete audit-tag`, true},
		{`git tag -a audit-tag -m feature -f`, true},
		{`git tag -a audit-tag -m --delete --force`, true},
		{`git tag -d audit-tag`, true},
		{`git tag --delete audit-tag`, true},
		{`git branch -m old-name new-name`, true},
		{`git branch -c old-name new-name`, true},
		{`git switch -C existing`, true},
		{`git checkout -B existing`, true},
		{`git reset --hard`, true},
		...
		{`gh --future-option=value pr view 123`, true},
		...
		{`gh pr checkout 123 --force=false -f`, true},
		...
		{`gh pr -R example/repository checkout 123 --force`, true},
		...
		{`gh repo -h view edit --visibility public`, true},
		...
		{`gh --force=false pr -f=true checkout 123`, true},
		{`gh --visibility public repo edit`, false},
		{`gh repo --description --visibility edit`, false},
		...
		{`gh pr view 123 --not-real`, true},
		{`gh pr --not-real value view 123`, true},
		{`gh pr -- checkout 123 --force`, false},
		{`gh pr view -- merge --force`, true},
		{`gh pr --repo "$repository" view 123`, true},
