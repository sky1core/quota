	...
	for _, model := range []string{"claude-fable-5-1", "fable", "claude-opus-4-8", "claude-sonnet-4-6"} {
		for _, tc := range []struct {
			...
			t.Run(model+"/"+tc.name, func(t *testing.T) {
				...
				_, err := selectClaudeAccount(config.Config{}, []string{"--model", model}, time.Now())
				if (err == nil) != allow {
					t.Errorf("direct selection error=%v, want allowed=%t", err, allow)
				}
				result, err := buildSelectAgentResult(config.Config{}, selectAgentOptions{agent: "claude", model: model}, time.Now())
				...
				_, err = selectAutoPromptAccount(config.Config{}, opts, catalogs, time.Now())
				if (err == nil) != allow {
...
func TestClaudeWeeklyModelUsesConfiguredFloor(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(fmt.Sprintf("%s/week%d/fable%d", tc.model, tc.weekly, tc.fable), func(t *testing.T) {
			...
			_, err := selectClaudeAccount(cfg, []string{"--model", tc.model}, time.Now())
			if (err == nil) != tc.allow {
...
func TestQuotaFailureListsEveryAccount(t *testing.T) {
	...
	_, err := selectClaudeAccount(cfg, []string{"--model", "fable"}, time.Now())
	if err == nil {
