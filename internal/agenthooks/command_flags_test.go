...
func TestCommandOptionValuesThroughHookEvent(t *testing.T) {
	...
	tests := []struct {
		...
		{`git tag -l -- --delete`, ""},
		{`git tag -a audit-tag -m '' --force`, ""},
		{`git tag -a audit-tag --message= --delete`, ""},
		{`git tag -a audit-tag -m -- -f`, ""},
		{`git tag -a audit-tag --message -- --delete`, ""},
		{`git tag -am--force audit-tag -f`, ""},
		{`git tag -famessage audit-tag`, ""},
		{`git tag -damessage audit-tag`, ""},
		{`git tag --del audit-tag`, ""},
		{`git tag --forc audit-tag`, ""},
		{`git tag --column --delete audit-tag`, ""},
		{`git tag -n -d audit-tag`, ""},
		{`git tag --no-file -f audit-tag`, ""},
		...
		{`git branch --contains -- --force feature`, ""},
		{`git branch --list -- --delete`, ""},
		{`git branch --list --end-of-options`, ""},
		{`git branch --list --end-of-options --delete`, ""},
		{`git branch --delete --end-of-options feature`, ""},
		{`git branch --format --end-of-options --delete feature`, ""},
		{`git tag -l --end-of-options --delete`, ""},
		{`git tag -m --end-of-options --delete audit-tag`, ""},
		{`git commit --end-of-options --amend`, ""},
		{`git commit -m --end-of-options --amend`, ""},
		{`git switch --end-of-options -Cfeature`, ""},
		{`git checkout --end-of-options -Bfeature`, ""},
		{`git reset --end-of-options --hard`, ""},
		{`git branch -m old new`, ""},
		{`git branch -qm old new`, ""},
		{`git branch -M old new`, ""},
		{`git branch -c old new`, ""},
		{`git branch -C old new`, ""},
		{`git branch --mov old new`, ""},
		{`git branch --cop old new`, ""},
		{`git branch --format '' -df feature`, ""},
		{`git branch --color -D feature`, ""},
		{`git branch -t -m old new`, ""},
		{`git branch --track --copy old new`, ""},
		...
		{`git switch -Cfeature`, ""},
		{`git switch -qCfeature`, ""},
		{`git switch --force-c=feature`, ""},
		{`git switch --create '' -Cfeature`, ""},
		{`git switch --recurse-submodules -Cfeature`, ""},
		...
		{`git checkout -qBfeature`, ""},
		{`git checkout -b '' -Bfeature`, ""},
		{`git checkout --track -Bfeature`, ""},
		{`git reset --pathspec-from-file --hard`, ""},
		{`git reset --pathspec-from-file=--hard`, ""},
		{`git reset --pathspec-from --hard`, ""},
		{`git reset -- --hard`, ""},
		{`git reset --pathspec-from-file '' --hard`, ""},
		{`git reset --pathspec-from-file -- --har`, ""},
		...
		{`gh co 12 -bfeature`, ""},
		{`gh pr checkout -- -f`, ""},
		{`gh pr checkout 12 -fbfeature`, ""},
		{`gh pr checkout 12 -bfeature -f`, ""},
		{`gh pr checkout 12 --branch= --force`, ""},
		{`gh pr checkout 12 -b= --force`, ""},
		{`gh pr checkout 12 --branch -- --force`, ""},
		{`gh pr checkout 12 --detach --force`, ""},
		...
		{`gh pr checkout 123 --force=false --force`, ""},
		{`gh pr checkout 123 --force=false -f`, ""},
		{`gh pr checkout 123 -f=false --force=TRUE`, ""},
		{`gh pr checkout 123 --force=1`, ""},
		{`gh pr checkout 123 --force false`, ""},
		{`gh pr checkout 123 --force --branch --force=false`, ""},
		{`gh pr checkout 123 --force -- --force=false`, ""},
		...
		{`gh pr close 12 --delete-branch=true`, "deny-gh-remote-code-ref-mutation"},
		{`gh pr close 12 -d=false`, ""},
		{`gh pr close 12 -d --delete-branch=false`, ""},
		{`gh pr close 12 --delete-branch=false -d`, "deny-gh-remote-code-ref-mutation"},
		{`git commit --no-t --dry-run --allow-empty -m example`, ""},
		{`git commit --no-tem --dry-run --allow-empty -m example`, ""},
		{`git commit --no-t --amend`, ""},
		{`git commit --trailer --amend -m example`, ""},

		{`gh pr checkout 12 -f=false`, ""},
		{`gh pr co 12 -b '' -f`, ""},
		{`gh co 12 -b '' -f`, ""},
		{`gh repo edit --description --visibility`, "deny-gh-remote-code-ref-mutation"},
		{`gh repo edit -d--visibility`, "deny-gh-remote-code-ref-mutation"},
		{`gh repo edit --description=--visibility`, "deny-gh-remote-code-ref-mutation"},
		{`gh repo edit --homepage --visibility`, "deny-gh-remote-code-ref-mutation"},
		{`gh repo edit -h--visibility`, "deny-gh-remote-code-ref-mutation"},
		{`gh repo edit -h= --visibility private`, "deny-gh-remote-code-ref-mutation"},
		{`gh repo edit --enable-issues=false`, "deny-gh-remote-code-ref-mutation"},
		{`gh repo edit -- --visibility`, "deny-gh-remote-code-ref-mutation"},
		{`gh repo edit --description '' --visibility=private`, "deny-gh-remote-code-ref-mutation"},
		{`gh repo edit --description -- --visibility private`, "deny-gh-remote-code-ref-mutation"},
		{`gh repo edit --enable-issues --visibility private`, "deny-gh-remote-code-ref-mutation"},
		{`command git -C . tag -amfeature audit-tag`, ""},
		{`sh -c 'git tag -m -- --delete audit-tag'`, ""},
...
func TestCommandOptionParsingReportsUnsupportedForms(t *testing.T) {
	for _, command := range []string{
		...
		t.Run(command, func(t *testing.T) {
			...
			if err != nil || len(invocations) != 1 {
				t.Fatalf("parse %s: %v, %+v", command, err, invocations)
			}
			parsed := invocations[0].command
			if parsed.flagError == "" && parsed.undecidable == "" {
				t.Fatalf("%s: missing option diagnostic", command)
			}
		})
	}
}
...
func TestCustomPolicyFlagValuesAndExceptions(t *testing.T) {
	...
	if err != nil || !decision.Allowed || decision.RuleID != "" {
		t.Fatalf("disabled local policy must not block a local command: decision = %+v, err = %v", decision, err)
	}
}
