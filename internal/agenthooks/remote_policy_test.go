...
func TestRemotePolicyBoundaryThroughHookEvents(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{`command "$cmd"`, false},
		{`nice -n "$priority" git status`, true},
		{`git add *.go`, true},
		{`git log "$BRANCH"`, true},
		...
		{`git -C {repo,push,origin} status`, false},
		...
		{`git for-each-repo --config=maintenance.repo rebase --onto $BASE main`, false},
		{`git bisect run true`, true},
		{`git bisect run git push origin main`, false},
		{`git bisect run env LANG="$LANG" git status`, true},
		{`git bisect run env LANG=$LANG git status`, false},
		...
		{`git submodule foreach git log "$BRANCH"`, true},
		...
		{`gh pr merge 12 --squash -b"$BODY" --disable-auto`, false},
		{`env -S 'gh pr merge 12 --squash' -b"$BODY" --disable-auto`, false},
		...
		{`gh pr close 12 "${PREFIX}"branch`, false},
		...
		{`env -S 'git add' $FILES`, true},
		{`env -S 'gh pr close' https://github.com/example/repo/pull/"$NUM"`, true},
		...
		{`env FOO="${arr[@]}" git status`, false},
		{`env "$KEY"=/tmp/startup bash -c true`, false},
		{`env BASH"$SUFFIX"=/tmp/startup bash -c true`, false},
		{`env -S 'sh -c' "$SCRIPT"`, false},
		{`env -S 'bash -O' "${opts[@]}" -c true`, false},
		{`env -S 'bash -O' {ext"glob",-c,"git push origin main"} -c true`, false},
		{`git submodule foreach bash -O "${opts[@]}" -c true`, false},
		{`git submodule foreach bash -O {ext"glob",-c,"git push origin main"} -c true`, false},
