...
// TestE2E_ReportToRenderedText wires the REAL parser to the REAL renderer, so the
...
func TestE2E_ReportToRenderedText(t *testing.T) {
	cases := []struct {
		name       string
		report     string
		wantRows   []string
		...
		{
			name: "today's real report",
			report: "Current session: 8% used · resets 5:50pm (Asia/Seoul)\n" +
				"Current week (all models): 2% used · resets Aug 20 at 12pm (Asia/Seoul)\n" +
				"Current week (Fable): 3% used · resets Aug 20 at 12pm (Asia/Seoul)\n",
			wantRows: []string{"Session", "Week", "Fable"},
		},
		{
			name: "Claude renames the period -> output follows, no code change",
			report: "Current session: 8% used\n" +
				"Current 5 days (all models): 2% used\n",
